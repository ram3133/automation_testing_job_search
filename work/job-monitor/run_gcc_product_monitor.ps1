...
$writeAppOutputs = if ($env:ROLEPILOT_WRITE_APP_OUTPUTS -match '^(1|true|yes|on)$') { $true } else { $false }
if (-not $env:ROLEPILOT_WRITE_APP_OUTPUTS) { $writeAppOutputs = $true }
$appPublicDir = if ($writeAppOutputs) { Join-Path $root "work\rolepilot-saas\public" } else { $null }
if ($writeAppOutputs) { New-Item -ItemType Directory -Force -Path $appPublicDir | Out-Null }
...
$appFeedPath = if ($writeAppOutputs) { Join-Path $appPublicDir "gcc_product_feed.js" } else { $null }
$appLinkedInFeedPath = if ($writeAppOutputs) { Join-Path $appPublicDir "linkedin_discovery_feed.js" } else { $null }
$ntfyTopic = if ($env:ROLEPILOT_NTFY_TOPIC) { [string]$env:ROLEPILOT_NTFY_TOPIC } else { "https://ntfy.sh/DE-Jobs" }
$minYears = if ($env:ROLEPILOT_MIN_YEARS) { [int]$env:ROLEPILOT_MIN_YEARS } else { 4 }
$maxYears = if ($env:ROLEPILOT_MAX_YEARS) { [int]$env:ROLEPILOT_MAX_YEARS } else { 7 }
...
$linkedInSearches = @(
  [ordered]@{
    ...
    url = "https://www.linkedin.com/jobs/search/?keywords=Data%20Engineer%20Databricks&location=Bengaluru%2C%20Karnataka%2C%20India&f_E=$minYears&f_TPR=r604800"
    cadence = "Daily"
  ...
  [ordered]@{
    ...
    location = "Bengaluru, Karnataka, India"
    url = "https://www.linkedin.com/jobs/search/?keywords=Azure%20Data%20Engineer&location=Bengaluru%2C%20Karnataka%2C%20India&f_E=$minYears&f_TPR=r604800"
    cadence = "Daily"
    lane = "LinkedIn Discovery"
  },
  [ordered]@{
    ...
    url = "https://www.linkedin.com/jobs/search/?keywords=Data%20Engineer%20Databricks&location=Hyderabad%2C%20Telangana%2C%20India&f_E=$minYears&f_TPR=r604800"
    cadence = "Daily"
    lane = "LinkedIn Discovery"
  },
  [ordered]@{
    ...
    role = "Azure Data Engineer"
    location = "Hyderabad, Telangana, India"
    url = "https://www.linkedin.com/jobs/search/?keywords=Azure%20Data%20Engineer&location=Hyderabad%2C%20Telangana%2C%20India&f_E=$minYears&f_TPR=r604800"
    cadence = "Daily"
    lane = "LinkedIn Discovery"
  },
  [ordered]@{
    ...
    url = "https://www.linkedin.com/jobs/search/?keywords=Data%20Engineer%20Azure%20Databricks&location=Kochi%2C%20Kerala%2C%20India&f_E=$minYears&f_TPR=r604800"
    cadence = "Daily"
    lane = "LinkedIn Discovery"
  },
  [ordered]@{
    ...
    url = "https://www.linkedin.com/jobs/search/?keywords=Data%20Engineer%20PySpark%20Databricks%20Azure&location=India&f_E=$minYears&f_TPR=r604800&f_WT=2%2C3"
    cadence = "Daily"
    lane = "LinkedIn Discovery"
  }
)
...
$profile = if ($env:ROLEPILOT_PROFILE) { $env:ROLEPILOT_PROFILE.ToLowerInvariant() } else { "data-engineering" }
switch ($profile) {
  "automation-testing" {
    $profileLabel = "Automation Testing"
    $profileMinYears = if ($env:ROLEPILOT_MIN_YEARS) { [int]$env:ROLEPILOT_MIN_YEARS } else { 4 }
    $profileMaxYears = if ($env:ROLEPILOT_MAX_YEARS) { [int]$env:ROLEPILOT_MAX_YEARS } else { 6 }
    $rolePattern = 'automation tester|automation test engineer|qa automation|sdet|test automation|software test engineer|quality engineer|automation test lead|test engineer|qa engineer|java automation|selenium engineer'
    $stackPattern = 'core java|java|selenium|selenium webdriver|webdriver|bdd|cucumber|testng|git|gitlab|github|sql|api testing|rest assured|postman|jenkins|maven|gradle|pytest|cypress|appium|junit|jest|automation|ui testing|functional testing|regression testing'
    $rejectPattern = 'intern|principal|director|manager|architect|training course|certification|question paper|walkin|walk-in|bpo|support|customer support|sales|account executive'
    $roleBoostKeywords = @("automation tester","automation test engineer","qa automation","sdet","test automation","software test engineer","quality engineer","test engineer","qa engineer","java automation","selenium engineer")
    $stackBoostKeywords = @("core java","selenium","selenium webdriver","webdriver","bdd","cucumber","testng","git","sql","api testing","postman","jenkins","rest assured","junit")
    $platformBoostKeywords = @("automation","ui testing","functional testing","regression testing","mobile testing","api automation","bdd framework","playwright","cypress","appium","mobile automation","web automation")
    $cloudBoostKeywords = @("aws","azure","gcp","devops","ci/cd","jenkins","gitlab","github actions","maven","gradle","rest assured","testng","junit")
    $genericAutomationKeywords = @("automation","qa","quality assurance","software testing","tester","test engineer","quality engineer","sdet","selenium","playwright","cypress","appium","testng","junit","cucumber","rest assured","api testing","functional testing","regression testing","ui testing","manual testing")
    $locationPattern = 'bengaluru|bangalore|hyderabad|kochi|pune|chennai|gurugram|noida|mumbai|india|india remote|remote india'
    $linkedInSearches = @(
      [ordered]@{ name = "Bengaluru Java Selenium"; role = "Java Automation Engineer"; location = "Bengaluru, Karnataka, India"; url = "https://www.linkedin.com/jobs/search/?keywords=Java%20Selenium%20Automation&location=Bengaluru%2C%20Karnataka%2C%20India&f_E=$profileMinYears&f_TPR=r604800"; cadence = "Daily"; lane = "LinkedIn Discovery" },
      [ordered]@{ name = "Bengaluru SDET"; role = "SDET"; location = "Bengaluru, Karnataka, India"; url = "https://www.linkedin.com/jobs/search/?keywords=SDET%20Java%20Selenium&location=Bengaluru%2C%20Karnataka%2C%20India&f_E=$profileMinYears&f_TPR=r604800"; cadence = "Daily"; lane = "LinkedIn Discovery" },
      [ordered]@{ name = "Hyderabad Java QA"; role = "Java QA Automation Engineer"; location = "Hyderabad, Telangana, India"; url = "https://www.linkedin.com/jobs/search/?keywords=Java%20QA%20Automation%20Engineer&location=Hyderabad%2C%20Telangana%2C%20India&f_E=$profileMinYears&f_TPR=r604800"; cadence = "Daily"; lane = "LinkedIn Discovery" },
      [ordered]@{ name = "Hyderabad TestNG Cucumber"; role = "Automation Test Engineer"; location = "Hyderabad, Telangana, India"; url = "https://www.linkedin.com/jobs/search/?keywords=TestNG%20Cucumber%20Automation&location=Hyderabad%2C%20Telangana%2C%20India&f_E=$profileMinYears&f_TPR=r604800"; cadence = "Daily"; lane = "LinkedIn Discovery" },
      [ordered]@{ name = "India Remote QA Java"; role = "QA Automation Engineer"; location = "India Remote"; url = "https://www.linkedin.com/jobs/search/?keywords=QA%20Automation%20Engineer%20Java%20Selenium&location=India&f_E=$profileMinYears&f_TPR=r604800&f_WT=2%2C3"; cadence = "Daily"; lane = "LinkedIn Discovery" }
    )
    $broadDiscoveryQueries = @(
      [ordered]@{ name = "Automation Testing India"; query = '("Core Java" OR Java OR Selenium OR "Selenium WebDriver" OR Cucumber OR TestNG OR Postman OR Jenkins OR "API Testing") ("Bengaluru" OR "Hyderabad" OR "Kochi" OR "India Remote" OR "India") (apply OR careers OR jobs OR hiring)'; maxResults = 25 },
      [ordered]@{ name = "Java Selenium Cucumber India"; query = '("Java" AND Selenium AND Cucumber) ("Bengaluru" OR "Hyderabad" OR "India") (job OR apply OR careers)'; maxResults = 25 },
      [ordered]@{ name = "QA Automation GCC India"; query = '("QA Automation" OR "SDET" OR "Automation Engineer" OR "Java Automation Engineer") ("GCC" OR "product company" OR "careers" OR "apply") ("India" OR "Bengaluru" OR "Hyderabad")'; maxResults = 20 },
      [ordered]@{ name = "Automation Testing Direct ATS India"; query = '("Automation Tester" OR "QA Automation Engineer" OR "SDET" OR "Java Automation Engineer") ("India" OR "Bengaluru" OR "Hyderabad" OR "Kochi" OR "Pune" OR "Chennai") (greenhouse OR lever OR workdayjobs OR smartrecruiters)'; maxResults = 25 },
      [ordered]@{ name = "Automation Testing Recent Posts"; query = 'site:linkedin.com/jobs/view ("Automation Tester" OR "QA Automation Engineer" OR "SDET" OR "Java Automation Engineer" OR "TestNG" OR "Cucumber") ("India" OR "Bengaluru" OR "Hyderabad" OR "Remote")'; maxResults = 25 }
    )
  }
  "backend-engineering" {
  ...
  default {
    ...
    $roleBoostKeywords = @("data engineer","senior data engineer","data engineer ii","azure data engineer","databricks engineer","data platform engineer","analytics engineer","etl developer","big data engineer","data pipeline engineer")
    $stackBoostKeywords = @("spark","pyspark","spark sql","databricks","delta lake","unity catalog","lakehouse")
    $platformBoostKeywords = @("azure","adf","data factory","adls")
    $cloudBoostKeywords = @("sql","python","airflow","etl","data pipeline","data pipelines")
    $locationPattern = 'bengaluru|bangalore|hyderabad|kochi|pune|chennai|gurugram|noida|india|india remote|remote india'
  }
}
...
function Score-Job($company, $title, $location, $text) {
  ...
  if ($profile -eq "automation-testing") {
    foreach ($kw in @("automation tester","automation test engineer","qa automation","sdet","test automation","software test engineer","quality engineer","test engineer","qa engineer","automation engineer","selenium engineer")) {
      if ($hay.Contains($kw)) { $fit += 14; $keywords.Add($kw) }
    }
    foreach ($kw in $genericAutomationKeywords) {
      if ($hay.Contains($kw)) { $fit += 6; $keywords.Add($kw) }
    }
    foreach ($kw in @("selenium","playwright","cypress","appium","testng","junit","cucumber","rest assured","api testing","ui testing","functional testing","regression testing","mobile automation","web automation")) {
      if ($hay.Contains($kw)) { $fit += 8; $keywords.Add($kw) }
    }
    foreach ($kw in @("core java","java","python","javascript","typescript","sql","git","jenkins","maven","gradle","aws","azure","gcp","ci/cd","bdd")) {
      if ($hay.Contains($kw)) { $fit += 3; $keywords.Add($kw) }
    }
    if ($hay -match 'automation|qa|test|testing') { $fit += 8 }
    if ($hay -match 'manual testing|functional testing|regression testing|ui testing|api testing') { $fit += 8 }
  } else {
...
function Send-NtfyJobAlert($job) {
  ...
  $body = @(
    ...
    "Experience: target $minYears-$maxYears years; verify JD"
    "Fit: $safeReason"
  ) -join "`n"
  $headers = @{
    Title = $safeTitle
...
function Test-DiscoveryCandidate($title, $link, $description) {
  ...
  if ($profile -eq "automation-testing") {
    if ($hay -notmatch 'automation|qa|test engineer|test automation|sdet|selenium|playwright|cypress|appium|testng|junit|cucumber|rest assured|api testing|functional testing|regression testing|ui testing|manual testing') { return $false }
